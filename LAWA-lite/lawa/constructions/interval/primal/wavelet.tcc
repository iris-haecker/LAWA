...
template <typename T, Construction Cons>
long
Wavelet<T,Primal,Interval,Cons>::minK(int j, const T &x) const
{
    const T xLeft  = support(j, basis.M1.left.lastIndex()).l2;

    if (x<=xLeft) {
        return basis.rangeJ(j).firstIndex();
    }
    return iceil((x*pow2i<T>(j+1) - basis.M1.lengths(0) - basis.M1.leftband.length())/2 + basis.M1.left.lastIndex() + 1);
}

template <typename T, Construction Cons>
long
Wavelet<T,Primal,Interval,Cons>::maxK(int j, const T &x) const
{
    const T xRight = support(j, pow2i<T>(j)-basis.M1.right.length()+1).l1;
    
    if (x>=xRight) {
        return basis.rangeJ(j).lastIndex();
    }

    return ifloor((x*pow2i<T>(j+1)+basis.d-1-basis.M1.lengths(0))/2 + basis.M1.left.lastIndex() + 1);
}
...
Wavelet<T,Primal,Interval,Cons>::support(int j, long k) const
{
    ...
    if (k>pow2i<T>(j)-basis.M1.right.length()) {
        return Support<T>(1-pow2i<T>(-j-1)*(basis.M1.lengths(k-1-pow2i<T>(j))),
                          T(1));
    }
