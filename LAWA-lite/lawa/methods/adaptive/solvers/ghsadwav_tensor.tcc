...
template <typename T, typename Basis, typename APPLY, typename RHS>
GHS_ADWAV_TENSOR<T,Basis,APPLY,RHS>::GHS_ADWAV_TENSOR(const Basis   &_basis,
                                                      APPLY         &_Apply,
                                                      RHS           &_F)
...
GHS_ADWAV_TENSOR<T,Basis,APPLY,RHS>::SOLVE(T      nuM1,
                                           T      _eps,
                                           int    NumOfIterations,
                                           T      H1norm)
{
    ...
    Coefficients<Lexicographical,T,Index2D> w_k, w_kP1, g_kP1;
    IndexSet<Index2D> Lambda_kP1;
    T nu_kM1 = nuM1;
    T nu_k   = 0.;
    T total_time=0.;
...
template <typename T, typename Basis, typename APPLY, typename RHS>
IndexSet<Index2D>
GHS_ADWAV_TENSOR<T,Basis,APPLY,RHS>::GROW(const Coefficients<Lexicographical,T,Index2D> &w,
                                          T nu_bar, T &nu)
{
    T zeta = 2.*(omega*nu_bar)/(1-omega);
    T r_norm = 0.;
    Coefficients<Lexicographical,T,Index2D> r, Aw, rhs;
    ...
    Coefficients<AbsoluteValue,T,Index2D> r_abs;
    r_abs = r;
...
template <typename T, typename Basis, typename APPLY, typename RHS>
Coefficients<Lexicographical,T,Index2D>
GHS_ADWAV_TENSOR<T,Basis,APPLY,RHS>::GALSOLVE(const IndexSet<Index2D> &Lambda,
                                              ...
                                              T delta, T tol)
{
    ...
    else if (d==4) {   J = -std::ceil((1./1.5)*std::log(tol/((3*tol+3*delta)*kappa))); }
    else           { assert(0); }
    ...
    for (const_set_it col=Lambda.begin(); col!=Lambda.end(); ++col, ++col_count) {
        IndexSet<Index2D> LambdaRowSparse = Apply.A.compression.SparsityPattern(*col, Lambda, J);
    ...
    Coefficients<Lexicographical,T,Index2D> r0, APPLY_Aw;
