...
typedef lawa::GeMatrix<FullStorage<double, ColMajor> >  RealGeMatrix;
...
main()
{
    ...
    dual.enforceBoundaryCondition<DirichletBC>();
    ...
    blas::mm(Trans, NoTrans, 1.0, M0, M0_, 0.0, A);
    cerr << "1  A = " << A << endl;

    // B = transpose(M0)*M1_;
    blas::mm(Trans, NoTrans, 1.0, M0, M1_, 0.0, B);
    cerr << "0  B = " << B << endl;

    //C = transpose(M1)*M0_;
    blas::mm(Trans, NoTrans, 1.0, M1, M0_, 0.0, C);
    cerr << "0  C = " << C << endl;

    //D = transpose(M1)*M1_;
    blas::mm(Trans, NoTrans, 1.0, M1, M1_, 0.0, D);
    cerr << "1  D = " << D << endl;
