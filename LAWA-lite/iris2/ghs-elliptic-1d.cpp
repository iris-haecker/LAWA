...
typedef LaplaceOperator1D<T>                        LaplaceBilinearForm1D;
typedef NoPreconditioner1D<T,Index1D>               Preconditioner1D;

typedef CompressionPDE1D<T,Basis1D>                 Compression1D;

//Matrix definitions
typedef MapMatrix<T,Index1D,LaplaceBilinearForm1D,Compression1D,
                  Preconditioner1D>                                 MA;
...
main(int argc, char *argv[])
{
    ...
    MyLaplaceOperator1D         Bil(d, d_, jMin);
    MyLaplacePreconditioner1D   P(Bil);
    MyLaplaceCompression1D      Compr(Bil);
