...
pythag(const T a, const T b)
{
    ...
    absb = fabs(b);
    if (absa>absb) {
        return absa*std::sqrt(1+SQR(T(absb/absa)));
    } else {
        return (absb == T(0) ? T(0) : absb*std::sqrt(1+SQR(T(absa/absb))));
    }
}
    ...
    GeMatrix<FullStorage<T,Order> > &V)
{
    ...
    for (i=1; i<=n; ++i) {
        ...
        anorm = std::max(anorm,T(fabs(s(i))+fabs(rv1(i))));
    }
    for (i=n; i>=1; --i) {
        if (i<n) {
            if (g!=0) {
